...
$sql="SELECT * FROM avs WHERE index_number='$index'"; 
...
$sql="select grade.name as g_name,grade.hall_charge as h_charge,subject.name as s_name,avs_salary_history.subject_fee as s_fee,avs_salary_history.subtotal as s_sub,avs_salary_history.student_count as s_count
      from avs_salary_history 
	  inner join grade
	  on avs_salary_history.grade_id=grade.id
	  inner join subject
	  on avs_salary_history.subject_id=subject.id
      where avs_salary_history.index_number='$index' and avs_salary_history.year='$year' and avs_salary_history.month='$month' and (_status='Advance' or _status='Total Salary')";

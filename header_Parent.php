...
if($my_type == 'Parent'){
	...
	$sql1="SELECT COUNT(id) FROM notification_history WHERE user_type='Parent' AND index_number='$my_index' AND _isread='0'";
...
$sql="SELECT * FROM notification_history WHERE user_type='Parent' AND index_number='$my_index' ORDER BY id DESC";
...
if($type=="Parent"){
	$sql="SELECT * FROM Parent WHERE index_number='$index'";
	$result=mysqli_query($conn,$sql);
	$row=mysqli_fetch_assoc($result);	
}
?> 
